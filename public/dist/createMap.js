...
export function getClosestPoint(startingPoint) {
...
export function createMap(points) {
    ...
    function generateRandomXValue(minX, maxX) {
        return Math.floor(Math.random() * (maxX - minX + 1) + minX);
    }
    function generateRandomYValue(minY, maxY) {
        return Math.floor(Math.random() * (maxY - minY + 1) + minY);
    }
    function wasPointGeneratedToCloseToAnother(generatedPoint, closestPoint) {
    ...
    if (direction === "left") {
        ...
        leftPoint.setPosition(generateRandomXValue(0, selectedPoint.x), selectedPoint.y);
        if (wasPointGeneratedToCloseToAnother(leftPoint, getClosestPoint(leftPoint)))
            return createMap(pointManager.points);
        ...
        topPoint.setPosition(selectedPoint.x, generateRandomYValue(0, selectedPoint.y));
        if (wasPointGeneratedToCloseToAnother(topPoint, getClosestPoint(topPoint)))
            return createMap(pointManager.points);
        ...
        bottomPoint.setPosition(selectedPoint.x, generateRandomYValue(selectedPoint.y, window.innerHeight - bottomPoint.height));
        if (wasPointGeneratedToCloseToAnother(bottomPoint, getClosestPoint(bottomPoint)))
            return createMap(pointManager.points);
